...
int msm_camera_flash(struct msm_sync *sync, int level)
{
	...
	return sync->sdata->camera_flash(flash_level);
}
		...
		gfp_t gfp)
{
...
#endif
		{
			msm_camera_flash(sync, MSM_CAMERA_LED_HIGH);
		}
	}
	if (qtype != MSM_CAM_Q_VFE_MSG)
		goto for_config;
	...
	case VFE_MSG_SNAPSHOT:
		if (sync->pp_mask & (PP_SNAP | PP_RAW_SNAP)) {
			...
			if (qcmd->on_heap)
				qcmd->on_heap++;
			pr_info("%s: got snapshot message"\
				"with LPM pp on_heap %d\n",
				__func__, qcmd->on_heap);
				break;
		}
		if (qcmd->on_heap)
